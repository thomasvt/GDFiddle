...
namespace GDFiddle.Ecs.Querying
{
    public class EntityQuery<TC1, TC2> : EntityQuery where TC1 : struct where TC2 : struct
    {
        ...
        public void VisitAll(EntityCallback2<TC1, TC2> action)
        {
            using (QueryManager.ClaimQueryLock(QueryMask))
            {
                foreach (var pool in GetApplicablePools())
                {
                    ...
                    {
                        Parallel.For(0, itemCount, i => action(pool.EntityIds[i], ref componentArray1.Records[i], ref componentArray2.Records[i]));
                    }
                    else
                    {
                        for (var i = 0; i < itemCount; i++)
                        {
                            action(pool.EntityIds[i], ref componentArray1.Records[i], ref componentArray2.Records[i]);
                        }
                    }
                }
            }
        ...
        /// </summary>
        public void VisitAllBulk(EntityCallbackBulk2<TC1, TC2> bulkAction)
        {
            using (QueryManager.ClaimQueryLock(QueryMask))
            {
                if (RunInParallel)
