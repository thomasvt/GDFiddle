    ...
    internal class EditorShell : ContentControl
    {
        public EditorShell(GraphicsDevice graphicsDevice, IMessageBus messageBus)
        {
            Content = new Grid
            {
                ...
                Children =
                {
                    { new GameView(graphicsDevice, messageBus) { Background = new Color(34, 34, 34) }, new GridProperties(0, 0) },
                    { new SidePanel(messageBus), new GridProperties(2, 0)},
                    { new GridSplitter {Background = new Color(34, 34, 34)}, new GridProperties(1, 0)}
                }
            };
        }
    }
